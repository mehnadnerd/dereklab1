...
//    printf("I am rank %i xdim %i ydim %i matsize %i starty %i\n", rankme, xdim_size, ydim_size, matsize, mystarty);
    ...
    if (debug_perf == 0) {
        int nextrank = rankright;
        if (coords[0] == 0) {
            // monarch
            printf("argument matrix 0\n");
            for (int i = 0; i < ydim_size; i++) {
                for (int j = 0; j < xdim_size; j++) {
                    printf("%lf ", au[i*xdim_size + j]);
                }
                printf("\n");
            }
            for (int i = 0; i < (dims[0] - 1); i++) {
                MPI_Recv(ai, xdim_size*ydim_size, MPI_DOUBLE, nextrank, xtag, topocomm, MPI_STATUS_IGNORE);
                for (int i = 0; i < ydim_size; i++) {
                    for (int j = 0; j < xdim_size; j++) {
                        printf("%lf ", ai[i*xdim_size + j]);
                    }
                    printf("\n");
                }
                c[0] = nextrank + 1;
                MPI_Cart_rank(topocomm, c, &nextrank);
            }
            fflush(stdout);
        }
        else {
            MPI_Send(au, xdim_size*ydim_size, MPI_DOUBLE, rankmonarch, xtag, topocomm);
        }
    ...
    for (int matrixnum = 1; matrixnum < num_arg_matrices; ++matrixnum) {
        ...
        if (debug_perf == 0) {
            if (coords[0] == 0) {
                ...
                for (int i = 0; i < xdim_size; i++) {
                    for (int j = 0; j < matrix_dimension_size; j++) {
                        int nextrank; c[0] = j/ydim_size;
                        MPI_Cart_rank(topocomm, c, &nextrank);
                        if (nextrank == 0) {
                            // monarch
                            printf("%lf ", bu[j*xdim_size + i]);
                        }
                        else {
                            double val;
                            MPI_Recv(&val, 1, MPI_DOUBLE, nextrank, xtag, topocomm, MPI_STATUS_IGNORE);
                            printf("%lf ", val);
                        }
                    }
                    printf("\n");
                ...
                for (int i = 0; i < xdim_size; i++) {
                    for (int j = 0; j < ydim_size; j++) {
                        double val = bu[j*xdim_size + i];
                        MPI_Send(&val, 1, MPI_DOUBLE, rankmonarch, xtag, topocomm);
                    }
                }
            }
            MPI_Barrier(topocomm);
...
//    debug_print_matrix(o, xdim_size, ydim_size, rankme);
    if (debug_perf == 0) {
        ...
        if (coords[0] == 0) {
            ...
            MPI_Send(o, xdim_size*ydim_size, MPI_DOUBLE, rankmonarch, xtag, topocomm);
        }
    }
    else {
        if (ammonarch) {
